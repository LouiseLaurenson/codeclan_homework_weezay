...
tweets %>% 
  ...
  summarise(sum(favorite_count)) %>% 
  arrange(desc(`sum(favorite_count)`))
...
tweets %>% 
  select(text) %>% 
  mutate(charcter_count = str_length(text)) %>% 
  summarise(mean(charcter_count))
...
```{r}
status <- read.csv("data/code_clan_info.csv")

tweet_status <- full_join(tweets, status, by = "tweet_id")
```
